...
For a fresh FASTWIN retrain, this backfill is the stage after `backtest_candidates.py --fastwin-full-pool` creates new market outcomes and before `scripts/finalize_fresh_authoritative_meta_pool.py` publishes the retraining pool. It must not convert a historical exact replay into a fresh pool; replay outputs remain explicitly historical.
